    ...
    <>
      ...
      <section className="mx-5 mb-4 bg-muted border border-surface2 rounded-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-surface2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-surface2 text-muted hover:text-lime-green hover:border-lime-green/30 transition-colors text-sm font-medium"
          >
            <Pencil size={13} />
          </button>
        </div>
      ...
      {isEditing && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
          <div className="bg-surface border border-surface2 rounded-2xl w-full max-w-md relative max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-surface2 sticky top-0 bg-surface z-10">
              <h3 className="font-syne font-bold text-base text-muted">
                Edit Profile
              </h3>
              <button
                onClick={() => setIsEditing(false)}
                className="w-8 h-8 rounded-xl bg-surface2 flex items-center justify-center text-muted hover:text-muted transition-colors"
              >
                <X size={15} />
              </button>
            </div>
